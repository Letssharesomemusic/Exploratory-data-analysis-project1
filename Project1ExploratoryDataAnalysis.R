#PLOT1
power_con<-household_power_consumption <- read.csv("D:/ladata/household_power_consumption.txt", header=FALSE, sep=";")
...
# Transforming Date and Time variables from characters to objects of type Date:
...
# Transforming the Date and Time vars from characters into objects of type Date and:
subpower_con$Date <- as.Date(subpower_con$Date, format="%d/%m/%Y")
subpower_con$Time <- strptime(subpower_con$Time, format="%H:%M:%S")
subpower_con[1:1440,"Time"] <- format(subpower_con[1:1440,"Time"],"2007-02-01 %H:%M:%S")
subpower_con[1441:2880,"Time"] <- format(subpower_con[1441:2880,"Time"],"2007-02-02 %H:%M:%S")
